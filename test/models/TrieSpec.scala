...
class TrieSpec extends Specification {
  ...
  "Trie" should {
    "lookup values by prefix" in {
      Trie("foo" -> "a", "food" -> "b", "fo" -> "c", "fod" -> "d").getAllWithPrefix("foo") must contain(exactly("a", "b"))
    }
    "allow storing multiple values at one key" in {
      Trie("foo" -> "a", "foo" -> "b").getAllWithPrefix("foo") must contain(exactly("a", "b"))
    }
    ...
    "allow adding elements" in {
      ...
      trie.getAllWithPrefix("") must contain(exactly("a", "b"))
    }
    "allow adding elements to an existing key" in {
      val trie = Trie("foo" -> "a") + ("foo" -> "b")
      trie.getAllWithPrefix("foo") must contain(exactly("a", "b"))
    }
    ...
    "allow adding elements to a parent key" in {
      ...
      trie.getAllWithPrefix("fo") must contain(exactly("a", "b"))
      trie.getAllWithPrefix("foo") must_== Seq("a")
    }
